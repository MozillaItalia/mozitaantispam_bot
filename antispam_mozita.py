        ...
        try:
            if type_msg != "BIC":
                ...
                if nousername and not (int(user_id) in WhiteList or int(user_id) in AdminList):
                    ...
                    bot.sendMessage(chat_id, "Un utente ("+str(user_id) +
                                    ") è stato cacciato perché non ha impostato alcun username.")
                    ...
                    if type_msg != "J" and type_msg != "L":
                        bot.deleteMessage(
                            telepot.message_identifier(messaggio))
                    ...
                    if type_msg == "J":
                        # Nuovo utente
                        bot.sendMessage(
                            chat_id, messaggio_benvenuto, reply_markup=new)
                ...
                if text == "/leggiregolamento" and type_msg == "BIC":
                    if user_id == int(BlackList[str(int(message_id)-1)]):
                        ...
                        TempList[str(int(message_id)-1)
                                 ] = BlackList[str(int(message_id)-1)]
                        print(TempList[str(int(message_id)-1)])
                        TempList_name[str(TempList[str(
                            int(message_id)-1)])] = BlackList_name[str(BlackList[str(int(message_id)-1)])]
                        del BlackList_name[str(
                            BlackList[str(int(message_id)-1)])]
                    ...
                    if user_id in WhiteList or user_id in AdminList:
                        user_name_temp = str(
                            msg['text'].split(",")[0]).lstrip("@")
                        if str(user_name_temp) in TempList_name.values():
                            user_id_temp = int(next(
                                (x for x in TempList_name if TempList_name[x] == str(user_name_temp)), None))
                            message_id_temp = int(
                            ...
                            del TempList_name[str(
                                TempList[str(int(message_id_temp)-1)])]
                        ...
                        if len(chat_name) > 0:
                            for gruppo_x in chat_name.keys():
                                try:
                                    ...
                                    bot.sendMessage(
                                        chat_id, "Messaggio inviato in \""+str(chat_name[gruppo_x])+"\"")
                                except Exception as e:
                                    print("Excep:05 -> "+str(e))
                                    bot.sendMessage(
                                        chat_id, "Non è stato possibile inviare il messaggio in: "+str(chat_name[gruppo_x]))
                            bot.sendMessage(
                                chat_id, "Messaggio inviato in tutti i gruppi abilitati")
                            esito = "OK"
                        else:
                            bot.sendMessage(
